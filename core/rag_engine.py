...
_ANALYSIS_INSTRUCTIONS = {
    ...
    "usecase_diagram": (
        ...
        "- Name use cases as VERB + NOUN actions (e.g. 'Take Quiz', 'Edit Profile'), NOT simply UI screen names\n"
        "- DON'T just connect the actor to every use case directly. Model the true relationships!\n"
        "- Use ONLY the aliases in relationships:\n"
        "  U --> UC1\n"
        "  UC1 ..> UC5 : <<include>>\n"
        "- NEVER use parentheses in relationships when using aliases\n"
        "- Show <<include>> for mandatory sub-steps (e.g., Action ..> Authenticate : <<include>>)\n"
        "- Show <<extend>> for optional expansions (e.g., Upload Photo ..> Edit Profile : <<extend>>)\n"
        ...
        "  @enduml\n\n"
        "DO NOT: Include any explanation text outside @startuml/@enduml.\n"
        "DO NOT: Just list screens or Activities. Group related capabilities into cohesive business use cases (max 10-15 total).\n"
        "DO NOT: Include any skinparam or styling — styling is handled automatically.\n"
    ),
    "package_diagram": (
