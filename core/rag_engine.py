...
_ANALYSIS_INSTRUCTIONS = {
    ...
    "class_diagram": (
        ...
        "GOAL: Someone unfamiliar with this project should understand the architecture at a glance.\n\n"
        "RULES:\n"
        ...
        "    }\n"
        "  }\n"
        "  LoginActivity --> UserRepository : fetches users\n"
        "  @enduml\n\n"
        "STYLING (include after @startuml):\n"
        "  skinparam classAttributeIconSize 0\n"
        "  skinparam classFontStyle bold\n"
    ...
    "sequence_diagram": (
        ...
        "EXAMPLE OUTPUT:\n"
        "  @startuml\n"
        "  title \"Sequence — User Login Flow\"\n"
        "  participant \"LoginActivity\" as LA <<Activity>>\n"
        "  participant \"FirebaseAuth\" as FA <<Service>>\n"
        "  participant \"UserRepository\" as UR <<Repository>>\n"
        "  LA -> FA : 1. signIn(email, password)\n"
        "  activate FA\n"
        "  FA --> LA : 2. AuthResult\n"
        "  deactivate FA\n"
        "  alt success\n"
        "    LA -> UR : 3. fetchUserProfile(uid)\n"
        "    UR --> LA : 4. User object\n"
        "  else failure\n"
        "    LA -> LA : 5. showError(message)\n"
        "  end\n"
        "  @enduml\n\n"
        "STYLING (include after @startuml):\n"
    ...
    "activity_diagram": (
        ...
        "EXAMPLE OUTPUT:\n"
        "  @startuml\n"
        "  title \"Activity Flow — App Launch\"\n"
        "  start\n"
        "  |User|\n"
        "  :Open App;\n"
        "  |App|\n"
    ...
    "state_diagram": (
        ...
        "EXAMPLE OUTPUT:\n"
        "  @startuml\n"
        "  title \"State Diagram — MainActivity Lifecycle\"\n"
        "  [*] --> Created : onCreate()\n"
        "  state Created {\n"
        "    Created : entry / initViews()\n"
        "  }\n"
        "  Created --> Started : onStart()\n"
        "  Started --> Resumed : onResume()\n"
        "  Resumed --> Paused : onPause() [userLeaves]\n"
        "  Paused --> Stopped : onStop()\n"
        "  Stopped --> [*] : onDestroy()\n"
        "  note right of Resumed : User is actively interacting\n"
        "  @enduml\n\n"
        "STYLING (include after @startuml):\n"
        "  skinparam state {\n"
        "    BackgroundColor #1a1a2e\n"
    ...
    "component_diagram": (
        ...
        "    [Firestore] <<Database>>\n"
        "  }\n"
        "  [LoginActivity] --> [AuthRepository] : validates credentials\n"
        "  [AuthRepository] --> [FirebaseAuth] : signIn()\n"
        "  [HomeActivity] --> [CourseAdapter] : displays courses\n"
        "  [CourseAdapter] --> [Firestore] : fetches course data\n"
        "  @enduml\n\n"
        "STYLING (include after @startuml):\n"
        "  skinparam component {\n"
        "    BackgroundColor #1a1a2e\n"
    ...
    "usecase_diagram": (
        ...
        "- Wrap system in rectangle: rectangle \"AppName\" { }\n"
        "- Define 5-7 use cases max with aliases: usecase \"Login\" as UC1\n"
        "- CRITICAL: ALWAYS wrap use case names in parentheses in relationships:\n"
        "  U --> (Login)      CORRECT\n"
        "  U --> Login         WRONG — syntax error\n"
        ...
        "  (Take Quiz) ..> (Browse Courses) : <<extend>>\n"
    ...
    "package_diagram": (
        ...
        "- Use different colors per layer for visual distinction\n"
        "- LABEL EVERY dependency arrow: ui ..> domain : calls\n"
        "- If a dependency violates clean architecture, add: note on link : Warning\n\n"
        "EXAMPLE OUTPUT:\n"
        "  @startuml\n"
        "  title \"Package Diagram — MyApp\"\n"
        "  package \"UI Layer\" <<Presentation>> #1e293b {\n"
        "    class LoginActivity\n"
        "    class HomeActivity\n"
        "  }\n"
        "  package \"Domain Layer\" <<Business>> #1a1a2e {\n"
        "    class UserRepository\n"
        "    class CourseRepository\n"
        "  }\n"
        "  package \"Data Layer\" <<Persistence>> #0f172a {\n"
        "    class FirestoreClient\n"
        "    class SharedPrefsManager\n"
        "  }\n"
        "  \"UI Layer\" ..> \"Domain Layer\" : uses\n"
        "  \"Domain Layer\" ..> \"Data Layer\" : reads/writes\n"
        "  @enduml\n\n"
        "STYLING (include after @startuml):\n"
        "  skinparam packageStyle rectangle\n"
        "  skinparam package {\n"
    ...
    "deployment_diagram": (
        ...
        "EXAMPLE OUTPUT:\n"
        "  @startuml\n"
        "  title \"Deployment Diagram — MyApp\"\n"
        "  node \"Android Device\" {\n"
        "    artifact \"MyApp.apk\" as app\n"
        "    database \"Room DB\" as localdb\n"
        "  }\n"
        "  cloud \"Firebase\" {\n"
        "    [Authentication] as auth\n"
        "    database \"Firestore\" as fs\n"
        "  }\n"
        "  node \"Chat Server\" as chat\n"
        "  app --> auth : HTTPS\\n(Firebase SDK)\n"
        "  app --> fs : HTTPS\\n(Firestore SDK)\n"
        "  app --> chat : HTTPS\\n(Retrofit)\n"
        "  app --> localdb : SQLite\n"
        "  note right of chat : Chatbot API backend\n"
        "  @enduml\n\n"
        "STYLING (include after @startuml):\n"
        "  skinparam node {\n"
        "    BackgroundColor #1a1a2e\n"
    ...
    "navigation_diagram": (
        ...
        "- Add 'note right of S1 : ...' OUTSIDE state blocks (NOT inside { }) for important entry conditions\n"
        "- NEVER put note directives inside state { } blocks — they must be at the top level\n"
        ...
        "EXAMPLE OUTPUT:\n"
        "  @startuml\n"
        "  title \"Navigation Map — Accentify\"\n"
        "  [*] --> Login : app launch\n"
        "  state \"Login\" as Login\n"
        "  state \"SignUp\" as SignUp\n"
        "  state \"Home\" as Home\n"
        "  state \"Lessons\" as Lessons\n"
        "  state \"Quiz\" as Quiz\n"
        "  state \"Settings\" as Settings\n"
        "  Login --> Home : successful login\n"
        "  Login --> SignUp : taps \"Sign Up\"\n"
        "  SignUp --> Home : account created\n"
        "  Home --> Lessons : selects course\n"
        "  Home --> Settings : taps profile\n"
        "  Lessons --> Quiz : taps \"Start Quiz\"\n"
        "  Quiz --> Home : quiz complete\n"
        "  Settings --> Home : taps back\n"
        "  Home --> [*] : presses back\n"
        "  note right of Login : Entry point\\nChecks cached session first\n"
        "  @enduml\n\n"
        "STYLING (include after @startuml):\n"
        "  skinparam state {\n"
        "    BackgroundColor #1a1a2e\n"
